...
export async function GET() {
  ...
  const [bookings, expenses, records, revisions, availability, messages, documents, settings, audit] = await Promise.all([
  ...
  return new NextResponse(JSON.stringify({ exportedAt: new Date().toISOString(), controller: 'APX RIDE', bookings: bookings.results, expenses: expenses.results, complianceRecords: records.results, recordRevisions: revisions.results, unavailablePeriods: availability.results, messageLogs: messages.results, documentIndex: documents.results, settings: settings.results, auditTrail: audit.results }, null, 2), {
