...
export async function GET() {
  ...
  const rows = await env.DB.prepare('SELECT id,email,role,active,created_at FROM portal_staff WHERE organisation_id=? AND owner_id=? ORDER BY role,email').bind(user.organisationId, user.ownerId).all();
